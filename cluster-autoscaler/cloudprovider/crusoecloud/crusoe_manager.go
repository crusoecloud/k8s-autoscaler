...
func (mgr *crusoeManager) Refresh() error {
	...
	for _, p := range pools {
		...
		spec, boundsSource := mgr.resolveNodeGroupSpec(&p)
		if spec == nil {
			continue
		}
		klog.V(4).Infof("Nodepool %s (%s): scaling bounds [%d, %d], source=%s",
			p.Name, p.Id, spec.MinSize, spec.MaxSize, boundsSource)
